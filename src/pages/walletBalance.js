...
function instructors(props) {
  ...
  const getProduct = async () => {
    ...
    Api("get", url, router).then(
      (res) => {
        ...
        setInstructorList(res.data.users);
  ...
  const actionHandler = ({ row }) => {
    return (
      <div
        className="flex bg-custom-sky px-4 cursor-pointer text-white items-center justify-evenly py-2 rounded-[10px] mr-[10px]"
        onClick={() => {
